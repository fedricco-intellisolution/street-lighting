...
export const ChecklistPendingCheck = () => {
    ...
    const generateDynamicColumns = useCallback(
        (response) => {
            ...
            response.checklist_item?.columns &&
                JSON.parse(response.checklist_item?.columns).forEach(
                    (element) => {
    ...
    const getPendingChecklist = useCallback(async () => {
        const response = await preventiveMaintenanceApi.getPendingChecklist(
            action
        );
    ...
    const submitPendingChecklist = async (data) => {
        ...
        try {
            const response =
                await preventiveMaintenanceApi.updatePendingChecklist(
                    action,
                    data
                );
                    ...
                    <Row className="pt-4">
                        <Col className="text-end">
                            <Button
                                ...
                                onClick={() =>
                                    navigate(
                                        "/preventive-maintenance/checklist-pending"
                                    )
                                }
                            >
                                Cancel
