...
const FileUploader = (props) => {
    const { control, name, disabled, label, setValue, data } = props;
    ...
    useEffect(() => {
        ...
        photos?.forEach((image) => {
            const exists = photoURLs.some(
                (el) =>
                    el.file_name === image.file_name ||
                    el.file_name === image.name
            );
            if (!exists) {
